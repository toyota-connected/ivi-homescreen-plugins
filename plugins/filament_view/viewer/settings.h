...
using ScreenSpaceReflectionsOptions =
    filament::View::ScreenSpaceReflectionsOptions;
...
using MultiSampleAntiAliasingOptions =
...
void applySettings(Engine* engine,
                   ...
                   const utils::Entity* sceneLights,
...
UTILS_PUBLIC
ColorGrading* createColorGrading(const ColorGradingSettings& settings,
                                 Engine* engine);
...
class UTILS_PUBLIC JsonSerializer {
 public:
  ...
  // Reads the given JSON blob and updates the corresponding fields in the given
  // Settings object.
...
// This defines fields in the same order as the setter methods in
// filament::View.
struct ViewSettings {
...
// This struct has a fixed size for simplicity. Each non-empty property name is
// an override.
struct MaterialSettings {
...
struct LightSettings {
  ...
  math::float3 sunlightColor =
      filament::Color::toLinear<filament::ACCURATE>({0.98, 0.92, 0.89});
