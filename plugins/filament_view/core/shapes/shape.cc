             ...
             const flutter::EncodableMap& params) 
{
  ...
  for (auto& it : params) {
    ...
    if (it.second.IsNull())
    {
      SPDLOG_WARN("Shape Param ITER is null {} {} {}", key.c_str(), __FILE__, __FUNCTION__);
      continue;
    }
    ...
    } else if (key == "shapeType" && std::holds_alternative<int32_t>(it.second)) {
...
void Shape::vRemoveEntityFromScene() {
  ...
  {
    SPDLOG_WARN("Attempt to remove uninitialized shape from scene {}::{}", __FILE__, __FUNCTION__);
    return;
  }
  CustomModelViewer::Instance("Shape")->getFilamentScene()->removeEntities(m_poEntity.get(), 1);
}

void Shape::vAddEntityToScene() {
  if(m_poEntity.get() == nullptr)
  {
    SPDLOG_WARN("Attempt to add uninitialized shape to scene {}::{}", __FILE__, __FUNCTION__);
    return;
  }
    
  CustomModelViewer::Instance("Shape")->getFilamentScene()->addEntity(*m_poEntity.get());
}
...
bool Shape::bInitAndCreateShape(::filament::Engine* engine_
  ...
  // Future tasking planned for all the types to be defined here, and create based
  // off settings sent in, for now only cube is represented.
...
, MaterialManager* material_manager) 
{
    ...
    // this will also set all the default values of the material instance from the material param list
    auto materialInstanceResult = material_manager->getMaterialInstance(m_poMaterial->get());

    RenderableManager::Builder(1)
        .boundingBox({{}, m_f3ExtentsSize}) 
        .material(0, materialInstanceResult.getData().value())
        .geometry(0, RenderableManager::PrimitiveType::TRIANGLES,
                  vertexBuffer, indexBuffer, 0, 36)
...
#else
void Shape::createCube(::filament::Engine* engine_, Entity* entityObject
, MaterialManager* material_manager) {
    static const float vertices[] = {
        -0.5f, -0.5f, 0.5f,   // Vertex 0
         0.5f, -0.5f, 0.5f,   // Vertex 1
